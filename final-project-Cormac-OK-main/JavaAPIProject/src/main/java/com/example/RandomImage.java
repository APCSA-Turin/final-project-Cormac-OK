...
public class RandomImage {
    ...
    public ImageObject randomImage(){
    ...
    public static ArrayList<ImageObject> parseJson(String data){
        JSONObject dataJSON = new JSONObject(data);
        JSONArray photosJSON = dataJSON.getJSONArray("photos");
        ArrayList<ImageObject> photosObj = new ArrayList<>();

        for(int i = 0; i < photosJSON.length(); i++){
            JSONObject photo = photosJSON.getJSONObject(i);
            photosObj.add(new ImageObject(
